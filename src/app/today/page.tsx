...
export default function TodayPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-white max-w-md mx-auto w-full min-h-100">
        ...
        <p className="text-sm font-medium text-title">Loading today&apos;s schedule...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col px-5 pt-4 pb-8 bg-white max-w-md mx-auto w-full space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-[48px] leading-tight font-normal tracking-tight text-title">
          {greeting}
        </h1>
        <p className="text-base text-title mt-2 font-normal">
          {subtitle}
        </p>
      </div>
      ...
      <div className="space-y-4">
        {items.length === 0 || isError ? (
          <div className="text-center py-12 text-title text-sm font-normal border border-dashed border-gray-200 rounded-3xl p-6">
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  <span className="text-[14px] font-medium text-title pt-0.5 w-14 shrink-0">
                    {item.start_time}
                  </span>

                  <div className="flex-1 pr-2">
                    <h2 className="text-[18px] font-semibold text-title leading-snug">
                      {item.title}
                    </h2>
                    ...
                    {isExpanded && (
                      <div className="mt-3 space-y-3.5 text-[14px]">
                        ...
                        {(hostName || meetingPoint) && (
                          <div className="grid grid-cols-[60px_1fr] gap-x-2 gap-y-1 text-title font-medium">
                            {hostName && (
                              <>
                            ...
                            {meetingPoint && (
                              <>
                                <span className="text-title font-normal">Meet</span>
                  ...
                  {hasExpandableContent && (
                    <button
                      ...
                      className="w-9 h-9 rounded-full bg-brand-purple-light flex items-center justify-center text-title shrink-0 hover:opacity-90 transition-opacity cursor-pointer"
